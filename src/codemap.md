...
- **Plugin / Extension registration**: `index.ts` is a thin bootstrap that creates a single `ConfigLoader` and calls each feature's `register*` factory. `registerContextView`, `registerCustomFooter`, and `registerModelSelect` are lazy-activated once per process when a `session_start` arrives with the feature enabled; `registerAutoSessionName` attaches its event listeners immediately.
...
1. **Bootstrap** (`index.ts`)
   - Creates a singleton `ConfigLoader`.
   - On every `session_start`, calls `config.initializeConfig(ctx)`; surfaces validation errors via `ctx.ui.notify`.
   - Calls `registerAutoSessionName`, `registerModelSelect`, `registerCustomFooter`, and `registerContextView`, each receiving the shared loader.
...
2. **Config loading** (`config-loader.ts`, `utils/path.util.ts`, `schemas/*.schema.ts`)
   ...
   - `PathUtil` owns the config/auth file path conventions and shell-style variable expansion.
...
- **@earendil-works/pi-coding-agent**: Provides `ExtensionAPI`, `ExtensionContext`, `ExtensionCommandContext`, events (`session_start`, `session_shutdown`, `before_agent_start`, `input`, `turn_start`, `message_start`, `message_end`, `context`, `agent_settled`), command registration (`pi.registerCommand`), UI notifications (`ctx.ui.notify`), footer injection (`ctx.ui.setFooter`), custom dialogs (`ctx.ui.custom`), session naming (`pi.setSessionName`/`getSessionName`), model selection (`pi.setModel`/`getThinkingLevel`), session management (`ctx.sessionManager`), and tool registries (`pi.getAllTools`/`pi.getActiveTools`).
- **@earendil-works/pi-ai**: Used for `completeSimple`, `Model<Api>`, `modelsAreEqual`, `getSupportedThinkingLevels`, and model-metadata types during title generation and model selection.
- **@earendil-works/pi-tui**: Provides the `Component`/`Focusable` contracts, `TUI`, `Theme`, `Input`, `KeybindingsManager`, and text-width helpers (`truncateToWidth`, `visibleWidth`, `fuzzyFilter`) used by the footer, model picker, and context view.
...
- **Cross-extension events**: `piVimKeyEventId` from `constants.ts` generates event ids consumed by `model_select` (`pi-qol.model_select`) and `context_view` (`pi-qol.context_view`).
