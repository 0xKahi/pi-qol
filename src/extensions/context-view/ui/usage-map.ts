 ...
 */
import type { ContextUsageSnapshot } from '../model';
...
export function buildUsageMap(
  ...
  const contextWindow = usage.reported?.contextWindow;
  if (contextWindow === undefined || !Number.isFinite(contextWindow) || contextWindow <= 0 || columns <= 0 || rows <= 0) return undefined;
  ...
  const bufferTokens = mapScale < contextWindow ? 0 : clamp(usage.autoCompactReserveTokens ?? 0, 0, contextWindow - windowOccupancy);
...
function createSegments(usage: ContextUsageSnapshot, estimatedTotal: number, occupiedCells: number): MapSegment[] {
...
function createCell(index: number, occupiedCells: number, bufferStart: number, segments: readonly MapSegment[]): UsageMapCell {
