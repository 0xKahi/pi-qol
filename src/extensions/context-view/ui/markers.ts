...
import { BODY_INDENT, fitLine } from '../../../libs/modal';
...
const MARKER_LEGENDS: Record<ContextMarker, MarkerLegend> = {
  highlighted: {
    ...
    color: 'syntaxNumber',
    explanation:
      ' parts are injected by extensions into pi’s system prompt. They are excluded from the' +
 ...
 */
export function markerLegendLines(theme: Theme, markers: Iterable<ContextMarker>, width: number): string[] {
  const shown = new Set(markers);
  return MARKER_ORDER.filter(marker => shown.has(marker)).flatMap(marker => bulletLines(theme, MARKER_LEGENDS[marker], width));
}
