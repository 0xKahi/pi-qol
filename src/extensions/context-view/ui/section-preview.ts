...
import { BODY_INDENT, calculateViewport } from '../../../libs/modal';
import type { InjectedReference, InjectionSection, JsonSpan } from '../model';
import { normalizeInlineText, normalizePreviewText } from '../text';
import { shiftJsonSpan } from './json-preview';
import { type ContextMarker, droppedMarker, guessMarker, markerLegendLines, movedMarker } from './markers';
 ...
 */
export function previewLegendLines(theme: Theme, contents: readonly SectionedContent[], layout: PreviewDescriptionLayout): string[] {
...
function contentBodyLines(
  ...
  return wrapTextWithAnsi(body, wrapWidth).map(line => (line === '' ? '' : `${BODY_INDENT}${line}`));
}
 ...
 */
function descriptionBlockRows(lines: readonly string[]): number {
