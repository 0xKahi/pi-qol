...
export class UsageView implements ModalTab {
  ...
  private headerLines(width: number): string[] {
    ...
    const fullMetadata = normalizedModel === '' ? summary : `${theme.fg('muted', normalizedModel)}${separator}${summary}`;
    ...
    if (zoomLabel !== undefined) {
      ...
      return [this.fit(title, width), '', this.fit(zoomLabel, width), '', this.fit(summary, width)];
    }
  ...
  private reportedSummary(): string {
    ...
    if (reported.tokens === undefined) {
      ...
      return this.theme.fg('text', `≈${formatTokens(this.usage.estimatedTokens)}/${contextWindow} (${percent})`);
    }
  ...
  private legendLine(row: LegendRow, columns: LegendColumns, width: number, selected: boolean): string {
    ...
    const percentPart = percent === '' ? '' : `${' '.repeat(LEGEND_VALUE_GAP)}${this.theme.fg(selected ? 'accent' : 'dim', percent)}`;
    return fitLine(`${left}${leader}${this.theme.fg(valueColor, tokens)}${tokenPadding}${percentPart}`, width);
  }
  ...
  private mapCell(cell: UsageMapCell): string {
    ...
    const glyph = cell.categoryId === 'compacted-data' ? COMPACTED_CELL : cell.fill === 'full' ? FULL_CELL : PARTIAL_CELL;
  ...
  private entryHeader(entry: UsagePreviewEntry): string {
    ...
    if (entry.invisibleReasoning !== undefined) {
      ...
      cells.push(theme.fg('dim', `+ ${label} ${marker}${formatTokens(tokens)} (${marker}${formatTokens(total)})`));
    }
    return cells.join(' ');
  }
