...
import type { TextSpan } from './model';
...
const CUSTOM_TOOLS_FILLER = 'In addition to the tools above, you may have access to other custom tools depending on the project.';
...
export function findSectionToolBlocks(prompt: string, sections: readonly PromptSection[]): LocatedPromptBlock[] {
  ...
  return sections.flatMap((section): LocatedPromptBlock[] => {
    ...
    const moved = sections.some(other => other.start < section.start && order.indexOf(other.name) > order.indexOf(section.name));
...
export function findPromptBlocks(
  ...
  const documentationStart = headerStarts(prompt, DOCUMENTATION_BLOCK.header, excluded).find(start => start < baseEnd);
  ...
  return found
    .map(block => ({
      ...block,
      moved:
        block.id !== DOCUMENTATION_BLOCK.id &&
        (block.start >= baseEnd ||
          found.some(
            other =>
              BASE_PROMPT_BLOCKS.findIndex(entry => entry.id === other.id) > BASE_PROMPT_BLOCKS.findIndex(entry => entry.id === block.id) &&
...
function hasKnownBullet(prompt: string, span: TextSpan, blockId: string, tools: readonly PromptBlockTool[]): boolean {
  const lines = new Set(prompt.slice(span.start + 1, span.end).split('\n'));
  const known =
    blockId === AVAILABLE_TOOLS_BLOCK.id
...
function includeFiller(prompt: string, end: number, blockId: string): number {
  ...
  return prompt.startsWith(CUSTOM_TOOLS_FILLER, start) && (fillerEnd === prompt.length || prompt[fillerEnd] === '\n') ? fillerEnd : end;
}
