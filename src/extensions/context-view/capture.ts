...
import { analyzeSystemPrompt, type PromptOptionsSlice, type ToolSlice, textTokens } from './measure';
import {
  AGGREGATE_SOURCE,
  buildSnapshot,
  type CaptureOrigin,
...
import { createProbeToken, type ProbeToken } from './probe-token';
import type { PromptSourceSlice } from './prompt-additions';
import { copySystemMessage, replaySystemMessages, systemMessageText } from './transcript';
...
export type ProbeOutcome = { readonly status: 'captured' } | { readonly status: 'failed'; readonly reason: string };
...
export class InitialCaptureState {
  ...
  private latestOptions: BuildSystemPromptOptions | undefined;
  ...
  /** Latest owned structured inputs for Usage, kept process-local only. */
  public get promptOptions(): BuildSystemPromptOptions | undefined {
  ...
  public prepare(options: BuildSystemPromptOptions, promptAtHandler?: string): void {
    this.latestOptions = structuredClone(options);
...
export class SilentProbeState {
   ...
   */
  public sanitizeMessage(message: ContextEvent['messages'][number]): ContextEvent['messages'][number] | undefined {
  ...
  public settle(captured: boolean): boolean {
    ...
    this.phase = 'settled';
    this.resolve(captured ? { status: 'captured' } : { status: 'failed', reason: 'Silent probe settled without a context snapshot.' });
    return true;
  }
   ...
   */
  private blankProbePrompt(message: Extract<ContextEvent['messages'][number], { role: 'user' }>): ContextEvent['messages'][number] | undefined {
   ...
   */
  private blankProbeAbort(message: Extract<ContextEvent['messages'][number], { role: 'assistant' }>): ContextEvent['messages'][number] | undefined {
    const isProbeAbort = message.stopReason === 'aborted' || (message.stopReason === 'error' && message.errorMessage === SETUP_ABORT_ERROR_MESSAGE);
...
export function buildUsageSnapshot(input: UsageSnapshotInput): InitialSnapshot {
  const patches = input.initial.groups
  ...
  const options = copyPromptOptions(input.options);
  const items = analyzeSystemPrompt(
    systemMessageText(state),
    {
      ...options,
...
export function mergeRequestOnlyMessages(snapshot: InitialSnapshot, initial: InitialSnapshot): InitialSnapshot {
  ...
  const items = [...snapshot.groups.flatMap(group => group.items), ...requestOnly];
 ...
 */
export function collectPromptSources(allTools: readonly ToolInfo[], commands: readonly SlashCommandInfo[]): PromptSourceSlice[] {
...
function addPromptSource(sources: Map<string, CollectedPromptSource>, sourceInfo: SourceInfo, name: string): void {
 ...
 */
export function measureInjectedMessages(messages: ContextEvent['messages'], baselineMessages: ContextEvent['messages']): InjectionItem[] {
  ...
  for (const [index, message] of messages.entries()) {
    ...
    items.push({
      id: message.role === 'custom' ? `message:${message.customType}:${occurrence}` : `message:context:${message.role}:${occurrence}`,
      phase: 'initial',
      kind: 'message',
...
function consumeMessageSignature(counts: Map<string, number>, message: ContextEvent['messages'][number]): boolean {
  const signature = JSON.stringify(message);
...
function messagePreview(message: ContextEvent['messages'][number]): MessagePreview {
  ...
  if (message.role === 'bashExecution') {
    ...
    return {
      text: typeof content === 'string' ? content : content.flatMap(block => (block.type === 'text' ? [block.text] : [])).join('\n'),
    };
  }
