 ...
 * unit-testable.
 */
import { type BuildSystemPromptOptions, type ContextEvent, estimateTokens, type InputSource, type ToolInfo } from '@earendil-works/pi-coding-agent';

import { analyzeSystemPrompt, type PromptOptionsSlice, type ToolSlice } from './measure';
import { AGGREGATE_SOURCE_ID, buildSnapshot, type CaptureOrigin, type InitialSnapshot, type InjectionItem, type InjectionSource } from './model';
...
type ProbePhase = 'idle' | 'waiting' | 'waiting-after-timeout' | 'running' | 'running-after-timeout' | 'failed' | 'settled';
...
export class InitialCaptureState {
  ...
  /** Own structured prompt inputs from every `before_agent_start`; freeze only the Initial preparation. */
...
export function mergeContextOnlyMessages(snapshot: InitialSnapshot, initial: InitialSnapshot): InitialSnapshot {
  const contextOnly = initial.groups.flatMap(group => group.items.filter(item => item.kind === 'message' && item.contextOnly === true));
  if (contextOnly.length === 0) return snapshot;
  const items = [...snapshot.groups.flatMap(group => group.items), ...contextOnly];
