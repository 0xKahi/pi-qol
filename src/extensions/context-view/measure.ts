 ...
 */
export function analyzeSystemPrompt(systemPrompt: string, options: PromptOptionsSlice, tools: ToolSlice[] = []): InjectionItem[] {
  ...
  if (footer !== undefined && footer.end < systemPrompt.length) {
    ...
    if (added.trim().length > 0) {
      items.push(createItem('prompt-addition:aggregate', 'prompt-addition', AGGREGATE_SOURCE, 'system prompt additions', added));
    }
  }
...
function measureContextFiles(base: string, options: PromptOptionsSlice, items: InjectionItem[], carvedSpans: Span[]): void {
  ...
  for (const filePath of options.contextFilePaths ?? []) {
    ...
    items.push(createItem(`context-file:${filePath}`, 'context-file', PI_SOURCE, abbreviateHome(filePath, options.homeDir), content));
  }
...
function measureAppendedPrompt(base: string, options: PromptOptionsSlice, items: InjectionItem[], carvedSpans: Span[]): void {
