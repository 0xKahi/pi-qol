...
import { type PromptAdditionOptions, splitPromptAdditions } from './prompt-additions';
import {
...
export function analyzeSystemPrompt(
  ...
  const blocks = usesCustomPrompt
    ...
    : addInjectedReferences(
        splitBasePromptParts(
          remaining,
          bodyBlocks.map(block => ({
            ...block,
            start: carve(body.slice(0, block.start), carvedSpans).length,
          })),
        ),
        body,
        carvedSpans,
  ...
  const references = footer === undefined ? [] : measurePromptAdditions(systemPrompt, footer.end, { ...additions, excluded: tailBlocks }, items);
...
function analyzePromptSections(
  ...
  const droppedIds = replaced
    ? BASE_PROMPT_BLOCKS.filter(block => !sections.some(section => SECTION_PARTS[section.name]?.id === block.id)).map(block => block.id)
    : [];
  ...
  for (const block of BASE_PROMPT_BLOCKS) {
    if (droppedIds.includes(block.id)) {
      parts.push({ ...block, kind: 'base-prompt', text: '', dropped: true, injectedReferences: lines.dropped.get(block.id) });
    }
  }
  for (const section of sections) {
    ...
    parts.push(measureSectionPart(prompt, section, spans, lines.carved, blocks.find(candidate => candidate.start === section.body.start)?.moved));
  }
...
function measureSectionPart(
  ...
  const localSpans = spans
    .filter(span => span.start >= start && span.end <= section.body.end)
    .map(span => ({ start: span.start - start, end: span.end - start }));
  const text = carve(prompt.slice(start, section.body.end), localSpans);
  const references = injected
    .filter(span => span.partId === block.id && span.start >= start && span.end <= section.body.end)
...
function measureGeneratedSection(name: string, body: string, options: PromptOptionsSlice, items: InjectionItem[]): boolean {
  ...
  if (name === 'skills' && body.includes('<available_skills>')) {
    ...
    const record = /<skill>\s*<name>(.*?)<\/name>\s*<description>([\s\S]*?)<\/description>\s*<location>(.*?)<\/location>/g;
    const skills: SkillSlice[] = [];
    for (const match of body.matchAll(record)) {
      const name = match[1];
      const description = match[2];
      const filePath = match[3];
      if (name === undefined || description === undefined || filePath === undefined) continue;
      skills.push({ name: decodeXml(name), description: decodeXml(description), filePath: decodeXml(filePath) });
    }
    if (skills.length === 0) return false;
    const children = skills
      .map(skill => createItem(`skill:${skill.name}`, 'skills', PI_SOURCE, skill.name, [skill.name, skill.description, skill.filePath].join('\n')))
...
function decodeXml(text: string): string {
  ...
  return text.replace(/&(amp|lt|gt|quot|apos);/g, (_match: string, name: string) => entities[name] ?? _match);
}
 ...
 */
function measurePromptAdditions(systemPrompt: string, start: number, options: PromptAdditionOptions, items: InjectionItem[]): InjectedReference[] {
...
function additionItemId(source: InjectionSource): string {
  return source.id === AGGREGATE_SOURCE.id ? 'prompt-addition:unattributed' : `prompt-addition:${source.label}`;
}
...
function measureTools(
  ...
  for (const tool of tools) {
    ...
    const droppedLines = droppedPromptLines(tool, ownedGuidelines).filter(line => droppedPartIds.includes(line.partId));
    ...
    const promptSections = [...droppedSections(droppedLines), ...carveToolPromptSections(carver, tool, ownedGuidelines, owner)];
...
function collectDroppedReferences(dropped: Map<string, InjectedReference[]>, lines: readonly DroppedLine[], owner: InjectedOwner): void {
  for (const line of lines) {
 ...
 * produced them.
 */
function carveToolPromptSections(carver: PromptCarver, tool: ToolSlice, ownedGuidelines: string[], owner: InjectedOwner): SectionDraft[] {
  const sections: SectionDraft[] = [];
  const snippet = tool.snippet === undefined ? undefined : carveInjectedLine(carver, carver.toolsBlock, `\n- ${tool.name}: ${tool.snippet}`, owner);
...
function createPromptCarver(base: string, carvedSpans: Span[], blocks: readonly LocatedPromptBlock[]): PromptCarver {
  return {
    base,
    toolsBlock: {
      partId: AVAILABLE_TOOLS_BLOCK.id,
 ...
 */
function carveInjectedLine(carver: PromptCarver, block: CarvedBlock, line: string, owner: InjectedOwner): Span | undefined {
    ...
    return {
      ...
      bullets:
        block.bullets === undefined
          ? undefined
          : {
              start: block.bullets.start + shift,
              end: block.bullets.end + shift,
            },
    };
  });
}
 ...
 */
function measureContextFiles(base: string, options: PromptOptionsSlice, items: InjectionItem[], carvedSpans: Span[]): void {
  ...
  for (const filePath of options.contextFilePaths ?? []) {
    ...
    children.push(createItem(`context-file:${filePath}`, 'context-file', PI_SOURCE, abbreviateHome(filePath, options.homeDir), content));
  }
 ...
 */
function carveAppendedPrompt(base: string, options: PromptOptionsSlice, carvedSpans: Span[]): string | undefined {
  const append = options.appendSystemPrompt;
  if (append === undefined || append.length === 0) return undefined;
  const generatedStarts = [findContextSectionSpan(base)?.start, findSkillsSpan(base)?.start].filter((start): start is number => start !== undefined);
 ...
 */
function addInjectedReferences(parts: PromptPart[], base: string, carvedSpans: Span[], injectedSpans: InjectedSpan[]): PromptPart[] {
 ...
 */
function droppedBasePromptParts(base: string, dropped: Map<string, InjectedReference[]>): PromptPart[] {
  const parts: PromptPart[] = [];
...
function appendPromptPart(parts: PromptPart[], block: Pick<PromptPart, 'id' | 'label' | 'moved'>, text: string): void {
...
function createSystemPromptItem(parts: readonly PromptPart[]): InjectionItem {
  ...
  const sections = allocateSectionTokens(
    parts.map(part => ({
      label: part.label,
      text: part.text,
...
function createItem(id: string, kind: InjectionKind, source: InjectionSource, label: string, text: string): InjectionItem {
  return {
    id,
    phase: 'initial',
    kind,
 ...
 */
function createAggregateItem(id: string, kind: InjectionKind, source: InjectionSource, label: string, children: InjectionItem[]): InjectionItem {
  return {
    ...
    sections: children.map((child, index) => childSection(child, index === 0 ? '' : CHILD_SEPARATOR)),
    children,
  };
}
...
function childSection(child: InjectionItem, separator: string): InjectionSection {
  ...
  return {
    ...
    tokens: child.tokens,
    jsonSpan: span === undefined ? undefined : { start: span.start + separator.length, end: span.end + separator.length },
  };
}
