...
function classifyMessages(messages: ContextEvent['messages'], contextOnly: readonly InjectionItem[]): UsageCategory[] {
  ...
  const toolOutput = aggregate(
    'tool-output',
    'Tool Output',
    withoutEmpty([...leavesFromMap('tool-result', toolResults), leaf('bash-executions', 'Bash Executions', bashExecutions)]),
  );
  return withoutEmpty([
...
function thinkingEntries(message: AssistantContextMessage): UsagePreviewEntry[] {
  ...
  const entries = texts.map(
    (text, index): UsagePreviewEntry => ({
      timestamp: message.timestamp,
      breadcrumb: texts.length > 1 ? ['assistant', `thinking ${index + 1}/${texts.length}`] : ['assistant'],
  ...
  if (first !== undefined) {
    ...
    entries[0] = {
      ...
      ...(invisibleReasoning === undefined ? {} : { visibleTokens: first.tokens, invisibleReasoning }),
    };
  }
  return entries;
}
...
function reportedReasoningTokens(message: AssistantContextMessage): number | undefined {
  ...
  return typeof reasoning === 'number' && Number.isFinite(reasoning) && reasoning >= 0 ? reasoning : undefined;
}
...
function contentToText(content: string | ReadonlyArray<{ type: string; text?: string }>): string {
  ...
  return content.map(block => (block.type === 'text' && block.text !== undefined ? block.text : `[${block.type}]`)).join('\n');
}
...
function appendEntry(totals: Map<string, UsagePreviewEntry[]>, key: string, entry: UsagePreviewEntry): void {
