...
import { AGGREGATE_SOURCE, extensionSource, type InjectionSource, type TextSpan } from './model';
 ...
 */
export function splitPromptAdditions(prompt: string, start: number, options: PromptAdditionOptions): PromptAdditionRun[] {
  const observed = options.promptAtHandler;
  const boundary = observed !== undefined && observed.length > start && prompt.startsWith(observed) ? observed.length : start;
  const regions = [
...
function guessOwner(text: string, sources: readonly PromptSourceSlice[]): PromptAdditionOwner {
  ...
  for (const source of sources) {
    ...
    if (
      containsPath(normalized, source.path) ||
      containsPath(normalized, source.baseDir) ||
 ...
 */
function guessTool(text: string, sources: readonly PromptSourceSlice[], owner: string): string | undefined {
  const matches = new Set<string>();
  for (const source of sources) {
