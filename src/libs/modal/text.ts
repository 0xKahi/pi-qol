...
export function calculateViewport(itemCount: number, terminalRows: number, fixedLineCount: number, extraLineCount = 0): Viewport {
...
export function wrapDescriptionLines(theme: Theme, text: string, color: ThemeColor, width: number): string[] {
  ...
  return (wrapped.length === 0 ? [''] : wrapped).map(line => truncateToWidth(theme.fg(color, `${indent}${line}`), width, ''));
}
