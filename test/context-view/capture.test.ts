...
import type { BuildSystemPromptOptions, ContextEvent, SlashCommandInfo, ToolInfo } from '@earendil-works/pi-coding-agent';
...
import {
  ...
  SilentProbeState,
} from '../../src/extensions/context-view/capture.ts';
...
function skill(name: string, disableModelInvocation: boolean): NonNullable<BuildSystemPromptOptions['skills']>[number] {
  return {
    name,
...
test('captureActiveTools uses the final active set', () => {
  const tools = captureActiveTools([tool('read', 'builtin'), tool('search', 'npm:web')], ['search'], { toolSnippets: { search: 'Search the web' } });
...
test("captureActiveTools keeps pi's active-tool order and drops repeated names", () => {
  const tools = captureActiveTools([tool('read', 'builtin'), tool('search', 'npm:web')], ['search', 'read', 'search'], {});
...
test('collectPromptSources rosters the names of extension tools and commands', () => {
  ...
  assert.deepEqual(
    sources.map(source => [source.source, source.names]),
    [
      ['npm:web', ['search']],
      ['npm:web', ['fetch']],
      ['npm:ask', ['/ask']],
...
test('copyPromptOptions owns decomposition metadata and keeps only visible skills', () => {
  ...
  assert.deepEqual(copied.skills, [{ name: 'visible', description: 'visible description', filePath: '/tmp/visible/SKILL.md' }]);
});
...
test('measureInjectedMessages attributes custom and request-only messages without session history', () => {
  ...
  const items = measureInjectedMessages([ordinaryUser, sessionCustom, requestCustom, injectedUser, blockUser], [ordinaryUser, sessionCustom]);
...
test('Initial capture omits opaque signatures from injected and transformed assistant previews', () => {
  ...
  const baselines: ContextEvent['messages'][] = [[], [{ ...message, content: [{ type: 'text', text: 'before context transformation' }] }]];
  for (const baselineMessages of baselines) {
    ...
    assert.ok(snapshot);
    const item = snapshot.groups.flatMap(group => group.items).find(entry => entry.kind === 'message');
    assert.ok(item);
...
test('Initial capture reports injected image sizes instead of retaining their payloads', () => {
  ...
  assert.deepEqual(JSON.parse(items[1]?.text ?? ''), [{ type: 'image', data: '<4B omitted>', mimeType: 'image/jpeg' }]);
...
test('captured summary previews omit envelope metadata without changing the content', () => {
  const messages = [
    {
      ...
      summary: 'We fixed image previews.\nNext: update the tests.',
      tokensBefore: 42_000,
      timestamp: 1_700_000_000_000,
    },
    {
      role: 'branchSummary',
      summary: '{"fromId":"actual summary content"}',
      fromId: 'INTERNAL_BRANCH_ID',
  ...
  assert.deepEqual(
    ...
    messages.map(message => message.summary),
  );
  messages.forEach((message, index) => {
    ...
    assert.equal(item.chars, item.text.length);
    assert.equal(item.tokens, estimateTokens(message));
    assert.equal(item.requestOnly, true);
  });
...
test('mergeRequestOnlyMessages carries only request-only mutations into Usage snapshots', () => {
  ...
  assert.deepEqual(
    merged.groups.flatMap(group => group.items).map(entry => entry.id),
    ['request-message'],
  );
...
test('InitialCaptureState refreshes pending options and freezes the first snapshot', () => {
  ...
  // The controller also needs the newest structured options as a fallback.
  assert.equal(state.promptOptions?.cwd, '/different');
});
