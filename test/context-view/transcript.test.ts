...
import { buildUsageSnapshot, InitialCaptureState, measureInjectedMessages } from '../../src/extensions/context-view/capture.ts';
import { buildSnapshot } from '../../src/extensions/context-view/model.ts';
import { replaySystemMessages, type SystemMessage, systemMessageText } from '../../src/extensions/context-view/transcript.ts';
...
const PATCH: SystemMessage = {
  role: 'system',
  content: 'Additional system text',
  timestamp: 2,
  sections: { rules: '<rules>\nNew rules\n</rules>', review: '<review>\nReview instructions\n</review>' },
  toolsAdded: [SECOND_TOOL],
...
function usageFor(messages: Parameters<typeof buildUsageSnapshot>[0]['messages'], patches: SystemMessage[] = []) {
  const snapshot = buildUsageSnapshot({
    systemPrompt: 'LIVE PROMPT MUST NOT REPLACE TRANSCRIPT',
    options: { cwd: '/current' },
    allTools: [CURRENT_TOOL],
...
test('system replay appends content, patches sections, and applies tool removals before additions', () => {
  ...
  const final: SystemMessage = {
    role: 'system',
    content: [{ type: 'text', text: 'Final text', textSignature: 'OPAQUE' }],
    timestamp: 3,
  ...
  assert.deepEqual(replaySystemMessages([{ role: 'system', content: '', timestamp: 1 }]), { content: '', sections: {}, tools: [] });
});
...
test('Usage counts replayed sections and declarations once instead of live state or patch history', () => {
  ...
  assert.equal(
    usage.categories.some(category => category.id === 'extension-messages'),
    false,
  );
  assert.equal(
    usage.categories.some(category => category.id === 'system-tools'),
    false,
  );
  ...
  assert.equal(
    ...
    usageFor([{ role: 'system', content: state.content, sections: state.sections, toolsAdded: state.tools, timestamp: 1 }]).estimatedTokens,
  );
});
...
test('Usage distinguishes legacy fallback from explicit removal of all prompt content and tools', () => {
  ...
  const removed: SystemMessage = {
    ...
    content: '',
    timestamp: 3,
    sections: { preamble: null, rules: null, cwd: null },
    toolsRemoved: [{ name: 'read' }],
  };
...
test('Usage follows restored branches and compaction checkpoints without old system-message duplicates', () => {
  ...
  assert.equal(usageFor(buildSessionContext(restored.getEntries(), root).messages).estimatedTokens, usageFor([INITIAL]).estimatedTokens);
  ...
  for (const id of ['system-prompt', 'custom-tools']) {
    assert.equal(
      compactedUsage.categories.find(category => category.id === id)?.tokens,
...
test('captured system previews include section content and omit opaque text signatures', () => {
  const message: SystemMessage = {
    role: 'system',
    content: [{ type: 'text', text: '', textSignature: 'OPAQUE_SYSTEM_SIGNATURE' }],
...
test('frozen request-only system patches replay in request order and do not count as Extensions', () => {
  const first: SystemMessage = {
    role: 'system',
    content: '',
    timestamp: 1,
    sections: { rules: `<rules>\n${'Long earlier rule '.repeat(50)}\n</rules>` },
  };
  const last: SystemMessage = {
    role: 'system',
    content: '',
    timestamp: 1,
    sections: { rules: '<rules>\nFinal short rule\n</rules>' },
...
test('Initial keeps the effective prompt while owning request-only system patch replay inputs', () => {
  ...
  const snapshot = state.finalize(() => ({
    ...
    messages: [INITIAL, patch],
    baselineMessages: [INITIAL],
    allTools: [],
    activeToolNames: [],
    origin: 'real-turn',
  }));
  ...
  const item = snapshot.groups.flatMap(group => group.items).find(entry => entry.systemMessage !== undefined);
  assert.ok(item);
  patch.sections = {};
  patch.toolsAdded = [];
