...
import type { InitialSnapshot, InjectionItem, UsageCategory } from '../../src/extensions/context-view/model.ts';
import { collectPreviewEntries, computeUsage, toReportedUsage } from '../../src/extensions/context-view/usage.ts';

/** Minimal measured item fixture. */
function item(id: string, kind: InjectionItem['kind'], tokens: number, native = true, children?: InjectionItem[]): InjectionItem {
  return {
    id,
    phase: 'initial',
    kind,
...
test('computeUsage classifies Initial components and live session messages without double-counting', () => {
  const messages: ContextEvent['messages'] = [
    ...
    { role: 'bashExecution', command: 'ls', output: '123456', exitCode: 0, cancelled: false, truncated: false, timestamp: 5 },
    {
      role: 'bashExecution',
      command: 'xx',
      output: 'yyyyyy',
  ...
  assert.equal(
    ...
    usage.categories.reduce((sum, entry) => sum + entry.tokens, 0),
  );
...
test('computeUsage drops empty categories and aggregates duplicate tool/custom message sources', () => {
  ...
  assert.deepEqual(
    toolOutput.children?.map(entry => [entry.id, entry.tokens]),
    [['tool-result:read', 3]],
  );
  assert.deepEqual(
...
test('computeUsage builds per-block preview entries with timestamps and breadcrumbs', () => {
  ...
  const messages: ContextEvent['messages'] = [
    ...
    assistantMessage(),
    multiBlock,
    { role: 'bashExecution', command: 'ls', output: 'out', exitCode: 0, cancelled: false, truncated: false, timestamp: 30 },
  ];
  const usage = computeUsage({ snapshot: snapshot(), messages });

  const userEntries = category(usage.categories, 'user-messages').entries ?? [];
  assert.deepEqual(
    userEntries.map(entry => [entry.timestamp, [...entry.breadcrumb], entry.text]),
    [[10, ['user'], 'hello there']],
  );
  ...
  assert.deepEqual(
    textEntries.map(entry => [...entry.breadcrumb]),
    [['assistant'], ['assistant', 'text 1/2'], ['assistant', 'text 2/2']],
  );
  ...
  assert.deepEqual(
    callEntries.map(entry => [entry.timestamp, [...entry.breadcrumb]]),
    [
      [2, ['assistant', 'read']],
      [20, ['assistant', 'read']],
      [20, ['assistant', 'bash']],
  ...
  assert.equal(
    callCategory.tokens,
    callEntries.reduce((sum, entry) => sum + entry.tokens, 0),
  );

  const bashEntries = category(usage.categories, 'bash-executions').entries ?? [];
  assert.deepEqual(
    bashEntries.map(entry => [entry.timestamp, [...entry.breadcrumb], entry.text]),
    [[30, ['bash'], '$ ls\nout']],
  );

  // Snapshot-backed categories carry timeless content entries.
  const promptEntries = category(usage.categories, 'system-prompt').children?.flatMap(child => child.entries ?? []) ?? [];
...
test("computeUsage counts pi's LLM-transform text for bash executions and summaries", () => {
  const messages: ContextEvent['messages'] = [
    { role: 'bashExecution', command: 'ls', output: '123456', exitCode: 0, cancelled: false, truncated: false, timestamp: 1 },
    { role: 'bashExecution', command: 'ls', output: '', exitCode: 0, cancelled: false, truncated: false, timestamp: 2 },
    { role: 'bashExecution', command: 'ls', output: 'err', exitCode: 2, cancelled: false, truncated: false, timestamp: 3 },
    { role: 'bashExecution', command: 'ls', output: 'partial', exitCode: undefined, cancelled: true, truncated: false, timestamp: 4 },
    {
      role: 'bashExecution',
      command: 'ls',
      output: '12345678',
  ...
  assert.deepEqual(
    bash.entries?.map(entry => [entry.timestamp, entry.tokens]),
    [
      [1, 6],
      [2, 5],
      [3, 12],
  ...
  assert.deepEqual(
    compacted.entries?.map(entry => [...entry.breadcrumb, entry.tokens]),
    [
      ['compaction', 29],
      ['branch', 26],
    ],
  ...
  assert.deepEqual(
    compacted.entries?.map(entry => entry.text),
    ['abcdefgh', 'abcd'],
  );
});
...
test('computeUsage uses provider reasoning per message and keeps signature estimates out of raw previews', () => {
  ...
  assert.deepEqual(
    entries.map(entry => entry.visibleTokens),
    [2, undefined, 2, undefined, 1],
  );
  assert.deepEqual(
    entries.map(entry => entry.invisibleReasoning),
    [
  ...
  assert.deepEqual(
    entries.slice(0, 2).map(entry => [...entry.breadcrumb]),
    [
