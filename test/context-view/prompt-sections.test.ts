...
import { analyzeSystemPrompt, type ToolSlice } from '../../src/extensions/context-view/measure.ts';
import { findPromptSections } from '../../src/extensions/context-view/prompt-blocks.ts';
...
test('XML sections after cwd remain named System Prompt parts, not guessed extension additions', () => {
  ...
  for (const options of [{ cwd: CWD }, { cwd: CWD, sections }]) {
    ...
    assert.equal(
      items.some(item => item.kind === 'prompt-addition'),
      false,
    );
    const base = items[0];
    assert.ok(base);
    assert.deepEqual(
      base.children?.slice(-2).map(part => [part.label, part.text]),
      Object.entries(sections),
    );
    assert.equal(
      base.sections?.reduce((sum, part) => sum + part.tokens, 0),
      base.tokens,
    );
...
test('custom overrides of every generated section retain their actual text', () => {
  const sections = {
    tools: 'Tools are unavailable.',
    rules: 'Follow the review checklist.',
    docs: 'Read the project manual.',
    addendum: 'Changed addendum',
    project_context: 'Replaced instructions',
    skills: 'Replaced skills',
    cwd: '/remote',
  };
  ...
  assert.deepEqual(
    items.find(item => item.kind === 'tool')?.sections?.map(part => part.label),
    ['Definition'],
  );
});
...
test('a custom prefix can restore selected native XML sections without marking them dropped', () => {
  ...
  assert.deepEqual(
    items[0]?.children?.filter(part => part.dropped).map(part => part.label),
    ['Guidelines'],
  );
...
test('section discovery ignores nested and fenced examples, including unclosed fences inside sections', () => {
  const prompt =
    ...
    '<cwd>\n/fixture\n</cwd>\n\n<review>\nReal section\n</review>';
  assert.deepEqual(
    findPromptSections(prompt).map(section => section.name),
    ['addendum', 'cwd', 'review'],
  );
  const parts = analyzeSystemPrompt(prompt, { cwd: CWD })[0]?.children ?? [];
  assert.equal(parts.find(part => part.label === 'Appended Prompt')?.text, '<rules>\nNested example\n</rules>\n```\nUnclosed inner fence');
...
test('unwrapped gaps around custom sections remain additions without absorbing section text', () => {
  const prompt =
    buildSystemPrompt({ cwd: CWD, sections: { review: 'Structured rule' } }).replace('<review>', 'First addition\n\n<review>') + '\n\nLast addition';
...
test('captured instruction files and skill records do not require current loader metadata', () => {
  ...
  assert.equal(items.find(item => item.id === 'skills')?.children?.[0]?.text, 'test\nTest & verify <code>\n/old/SKILL.md');
