...
import { analyzeSystemPrompt, type PromptOptionsSlice, type ToolSlice, textTokens } from '../../src/extensions/context-view/measure.ts';
import { buildSnapshot, type InjectionItem } from '../../src/extensions/context-view/model.ts';
import { collectPreviewEntries, computeUsage } from '../../src/extensions/context-view/usage.ts';
...
test('analyzeSystemPrompt emits stable semantic ids and content-only measurements', () => {
  ...
  assert.deepEqual(
    ...
    ['base-prompt', 'tool:npm:web:search', 'tool:builtin', 'context-files', 'skills', 'prompt-addition:unattributed'],
  );
  ...
  assert.equal(
    skills?.chars,
    skills?.children?.reduce((sum, child) => sum + child.chars, 0),
  );
  assert.equal(
    skills?.tokens,
    skills?.children?.reduce((sum, child) => sum + child.tokens, 0),
  );
...
test('analyzeSystemPrompt breaks tool items into reconciling prompt and definition sections', () => {
  ...
  assert.equal(
    search?.sections?.reduce((sum, section) => sum + section.tokens, 0),
    search?.tokens,
  );

  const builtin = items.find(entry => entry.id === 'tool:builtin')?.children?.[0];
  assert.deepEqual(
    builtin?.sections?.map(section => section.label),
    ['Definition'],
  );
...
test('analyzeSystemPrompt breaks the System Prompt into the parts pi assembles it from', () => {
  ...
  assert.deepEqual(
    base?.sections?.map(section => section.label),
    parts,
  );
  assert.deepEqual(
  ...
  assert.equal(
    base?.sections?.reduce((sum, section) => sum + section.tokens, 0),
    base?.tokens,
  );
  assert.equal(
    base?.children?.reduce((sum, child) => sum + child.tokens, 0),
    base?.tokens,
  );
  assert.deepEqual(
...
test('analyzeSystemPrompt gives a repeated guideline bullet to the tool pi renders it for', () => {
  ...
  assert.deepEqual(
    fetch?.sections?.map(section => section.label),
...
test('Available Tools references restore extension snippets without changing counted text', () => {
  ...
  assert.deepEqual(
    references.map(reference => [reference.text, reference.itemId, reference.source.label]),
    [['\n- search: Search the web', 'tool:npm:web:search', 'npm:web']],
  );
  assert.deepEqual(base.sections?.find(section => section.label === 'Available Tools')?.injectedReferences, references);
...
test('guideline references restore prompt order without changing counted text or token shares', () => {
  ...
  assert.equal(
    usage.categories.find(category => category.id === 'custom-tools')?.tokens,
...
test('guideline attribution matches complete bullets rather than shorter prefixes', () => {
  ...
  assert.deepEqual(
    references?.map(reference => reference.source.label),
    ['npm:long', 'npm:short'],
  );
  ...
  assert.deepEqual(
    findItem(missing, 'tool:npm:short:short')?.sections?.map(section => section.label),
    ['Definition'],
  );
...
test('analyzeSystemPrompt does not attribute context-file lines as custom-prompt tool guidance', () => {
  ...
  const systemPrompt = ['CUSTOM PROMPT', contextBlock, `Current date: ${currentDate()}`, `Current working directory: ${CWD}`].join('\n');
  const tools: ToolSlice[] = [
    {
      name: 'search',
      description: 'Search',
...
test('analyzeSystemPrompt marks the parts a replaced prompt drops without counting them', () => {
  ...
  assert.deepEqual(
    droppedChildren.map(child => child.label),
    ['Available Tools', 'Guidelines', 'Documentation'],
  );
...
test('analyzeSystemPrompt recovers the prompt blocks an extension relocated past the footer', () => {
  ...
  const systemPrompt = relocateToolSurface(buildRelocationPrompt()).replace(/\n\n<tools>/, `\n\n${addition}\n\n<tools>`);
...
test('analyzeSystemPrompt recognizes the pi 0.81 CWD-only footer', () => {
  const extensionAddition = '\nEXTENSION INSTRUCTION';
  const systemPrompt = ['BASE PROMPT', `Current working directory: ${CWD}`].join('\n') + extensionAddition;
...
test('analyzeSystemPrompt abbreviates home-directory context-file labels with ~', () => {
  ...
  const systemPrompt = ['BASE PROMPT', contextBlock, `Current date: ${currentDate()}`, `Current working directory: ${CWD}`].join('\n');
  const options: PromptOptionsSlice = {
    cwd: CWD,
    homeDir,
    contextFilePaths: [filePath],
...
test('analyzeSystemPrompt groups every context file under one Instruction Files aggregate', () => {
  ...
  assert.deepEqual(
    instructions?.sections?.map(section => section.label),
    [projectPath, '~/.pi/agent/AGENTS.md'],
  );
  assert.equal(
    instructions?.chars,
    instructions?.children?.reduce((sum, child) => sum + child.chars, 0),
  );
  assert.equal(
    instructions?.tokens,
    instructions?.children?.reduce((sum, child) => sum + child.tokens, 0),
  );
