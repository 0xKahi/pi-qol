...
import type { ContextUsageSnapshot, UsageCategory } from '../../src/extensions/context-view/model.ts';
import { buildUsageMap, calculateFitMapScale } from '../../src/extensions/context-view/ui/usage-map.ts';

/** Minimal usage fixture for proportional map tests. */
function usage(categories: UsageCategory[], reported: ContextUsageSnapshot['reported'], autoCompactReserveTokens?: number): ContextUsageSnapshot {
  return {
...
test('calculateFitMapScale adds headroom and rounds upward within its floor and window cap', () => {
  assert.equal(calculateFitMapScale(usage([{ id: 'messages', label: 'Messages', tokens: 100_000 }], { contextWindow: 1_000_000 })), 120_000);
  assert.equal(calculateFitMapScale(usage([{ id: 'messages', label: 'Messages', tokens: 43_800 }], { contextWindow: 1_000_000 })), 51_000);
  assert.equal(calculateFitMapScale(usage([{ id: 'messages', label: 'Messages', tokens: 200_000 }], { contextWindow: 1_000_000 })), 230_000);
  assert.equal(calculateFitMapScale(usage([{ id: 'messages', label: 'Messages', tokens: 1 }], { contextWindow: 1_000_000 })), 10_000);
  assert.equal(calculateFitMapScale(usage([{ id: 'messages', label: 'Messages', tokens: 1 }], { contextWindow: 8_000 })), 8_000);
  assert.equal(calculateFitMapScale(usage([{ id: 'messages', label: 'Messages', tokens: 900_000 }], { contextWindow: 1_000_000 })), 1_000_000);
...
test('buildUsageMap changes only the denominator for a smaller anchored scale', () => {
  ...
  assert.equal(fit.blockTokens, 6);
...
test('buildUsageMap works when pi usage is unknown', () => {
  const map = buildUsageMap(usage([{ id: 'messages', label: 'Messages', tokens: 45 }], { contextWindow: 100 }), 10, 1);
...
test('buildUsageMap marks trailing auto-compact buffer cells after free space', () => {
  const map = buildUsageMap(usage([{ id: 'messages', label: 'Messages', tokens: 40 }], { contextWindow: 100 }, 20), 10, 1);

  assert.ok(map !== undefined);
  assert.deepEqual(
    map.cells.map(cell => cell.fill),
    ['full', 'full', 'full', 'full', 'free', 'free', 'free', 'free', 'buffer', 'buffer'],
  );
...
test('buildUsageMap leaves the true-window buffer beyond a smaller Fit range', () => {
  const map = buildUsageMap(usage([{ id: 'messages', label: 'Messages', tokens: 40 }], { contextWindow: 100 }, 100), 10, 1, 50);

  assert.ok(map !== undefined);
  assert.deepEqual(
    map.cells.map(cell => cell.fill),
    ['full', 'full', 'full', 'full', 'full', 'full', 'full', 'full', 'free', 'free'],
  );
...
test('buildUsageMap shrinks the buffer when content grows into the reserve', () => {
  ...
  const map = buildUsageMap(usage([{ id: 'messages', label: 'Messages', tokens: 90 }], { contextWindow: 100 }, 20), 10, 1);

  assert.ok(map !== undefined);
  assert.deepEqual(
    map.cells.slice(8).map(cell => cell.fill),
    ['full', 'buffer'],
  );
});

test('buildUsageMap omits buffer cells when auto-compaction is disabled', () => {
  const map = buildUsageMap(usage([{ id: 'messages', label: 'Messages', tokens: 40 }], { contextWindow: 100 }), 10, 1);
