...
import { type PromptSourceSlice, splitPromptAdditions } from '../../src/extensions/context-view/prompt-additions.ts';
...
function split(addition: string, sources: readonly PromptSourceSlice[] = [], promptAtHandler?: string): Array<[string, string, string | undefined]> {
...
function owners(addition: string, sources: readonly PromptSourceSlice[]): Array<[string, string, string | undefined]> {
...
test('splitPromptAdditions attributes blank-line blocks separately and merges same-source neighbours', () => {
  assert.deepEqual(split('\n\nPirate mode.\n\nRead @eko24ive/pi-ask docs.\n\nStill pi-ask territory: @eko24ive/pi-ask.', [ASK]), [
    ...
    ['\n\nRead @eko24ive/pi-ask docs.\n\nStill pi-ask territory: @eko24ive/pi-ask.', 'npm:@eko24ive/pi-ask', 'guess'],
  ]);
});
...
test('recovered block exclusions preserve handler boundaries and keep source evidence separate', () => {
  ...
  for (const boundary of [FOOTER.length, start, start + 5, end, prompt.length]) {
    ...
    assert.deepEqual(
      runs.map(run => run.source.label),
      [WEB.source, 'unattributed'],
    );
  }
});
...
test('splitPromptAdditions excludes several recovered blocks without inventing whitespace owners', () => {
  ...
  assert.deepEqual(
    splitPromptAdditions(prompt, FOOTER.length, {
      excluded: [
