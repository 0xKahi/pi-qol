...
import { expandJsonSpan, shiftJsonSpan } from '../../src/extensions/context-view/ui/json-preview.ts';
...
test('expandJsonSpan indents only the marked run of a preview text', () => {
  ...
  assert.equal(
    ...
    ['search: Search the web', '{', '  "type": "object",', '  "properties": {', '    "q": {', '      "type": "string"', '    }', '  }', '}'].join(
      '\n',
    ),
  );
