...
import { findPromptBlocks } from '../../src/extensions/context-view/prompt-blocks.ts';
...
test('prompt blocks follow their actual order before or after the footer', () => {
  for (const prompt of [
    ...
    assert.deepEqual(
      ...
      ['Documentation', 'Available Tools', 'Guidelines'],
    );
    assert.deepEqual(
      blocks.filter(block => block.moved).map(block => block.label),
  ...
  assert.deepEqual(
    swapped.map(block => block.label),
    ['Guidelines', 'Available Tools', 'Documentation'],
  );
...
test('recovery needs contiguous complete bullets, not headers, prose, or prefix matches', () => {
  for (const invalid of [
    ...
    assert.deepEqual(
      locate(prompt).map(block => block.label),
      ['Documentation'],
    );
  }
});
...
test('pre-footer copies win; ambiguous post-footer duplicates remain additions', () => {
  ...
  assert.deepEqual(
    locate(duplicated).map(block => block.label),
    ['Documentation'],
  );
});
