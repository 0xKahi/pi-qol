...
import { buildUsageSnapshot, InitialCaptureState } from '../../src/extensions/context-view/capture.ts';
import type { InitialSnapshot, InjectionItem } from '../../src/extensions/context-view/model.ts';
import type { SystemMessage } from '../../src/extensions/context-view/transcript.ts';
...
function captureRun(options: BuildSystemPromptOptions, promptAtHandler: string, systemPrompt: string): InitialSnapshot {
...
test('Initial captures a forced prompt instead of the recorded sections, in both extension orders', () => {
  ...
  const orders = {
    ...
    before: captureRun({ ...OPTIONS, forceSystemPrompt: FORCED_SYSTEM_PROMPT }, FORCED_SYSTEM_PROMPT, FORCED_SYSTEM_PROMPT),
...
test("a forced prompt extending Pi's sections keeps the section parts and its addition, in both orders", () => {
  ...
  for (const promptAtHandler of [forced, structured]) {
    ...
    assert.deepEqual(
      ...
      ['Preamble', 'Available Tools', 'Guidelines', 'Documentation', 'Appended Prompt', 'Current Dir', 'Extension Additions'],
      order,
    );
...
test('Usage reads the recorded sections after a forced run, not the frozen forced prompt', () => {
  ...
  const initial = captureRun({ ...OPTIONS, forceSystemPrompt: FORCED_SYSTEM_PROMPT }, FORCED_SYSTEM_PROMPT, FORCED_SYSTEM_PROMPT);
  ...
  const previews = usage.categories
